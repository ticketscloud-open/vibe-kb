...
| 3   | resource    | varchar(255) | Источник          | ticket / customer / promo / event / push / campaign / oranizer / city                                                                                                                                                                |
...
ticket.order_id              → systemorder.external_id   (подтверждено: 3441 несовпадение из 11.5 млн — незначительная погрешность)
...
-- ticketorder.transaction_id: связь с tickettransaction.id НЕ подтвердилась (0 совпадений), природа поля не выяснена
...
-- funds.transaction_id: связь с tickettransaction.id НЕ подтвердилась (0 совпадений), природа поля не выяснена
...
systemoperation.transaction_id  → customertransaction.id   (подтверждено: 0 сирот)
...
adminlog.source_id           → customer.phone (когда resource = 'customer', уточнить — возможно customer.id)
