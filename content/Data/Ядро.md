...
| 12  | tl_disabled     | Bool               | Отключена ли автоматическая передача на витрину TicketLand (на уровне меты) | true / false             |
...
| 8   | currency                        | FixedString(3)            | Валюта                                                                          | RUB / EUR / GBP / KZT / USD / null |
...
| 43  | require_promo_materials         | Bool                      | Запрашивает ли расп промоматериалы                                              | true / false                       |
| 44  | accounts                        | Map(String, Int32)        | id аккаунтов                                                                    | {"RUB":743}                        |
...
| 9   | partner       | FixedString(24)    | id партнера   | 54e7b31a9cb5385cdc0706f1 |
| 10  | status        | String             | Статус        | new / removed            |
...
| 7   | obj             | FixedString(24)           | id объекта          | 5cf4fa8bb933cc000c74c9a8                                         |
| 8   | subj            | FixedString(24)           | id субъекта         | 5bf2af6c640440000c1f12ae                                         |
...
| 8   | status              | LowCardinality(String)           | Статус промоматериала           | accepted / rejected |
...
| #   | Поле     | Тип               | Описание                    | Пример                             |
| --- | -------- | ----------------- | --------------------------- | ---------------------------------- |
| 1   | id       | Int32             | id счёта                    | 77                                 |
| 2   | currency | Int16             | id валюты (`currencies.id`) | 643                                |
| 3   | amount   | Nullable(Float64) | Баланс (коп.)               | 500000                             |
| 4   | type     | Int16             | Тип счёта                   | world - 0 / core - 1 / partner - 2 |
...
| #   | Поле       | Тип              | Описание               | Пример           |
| --- | ---------- | ---------------- | ---------------------- | ---------------- |
| 1   | id         | Int64            | id валюты              | 643              |
| 2   | num_code   | String           | Числовой код           | 643              |
| 3   | alpha_code | String           | Буквенный код          | Russia Ruble     |
| 4   | name_ru    | Nullable(String) | Название на русском    | Рубль            |
| 5   | name_en    | Nullable(String) | Название на английском | Российский рубль |
| 6   | char       | Nullable(String) | Символ валюты          | ₽                |

---
#### transfers
Движения денег между счетами.

| #   | Поле         | Тип      | Описание                          | Пример              |
| --- | ------------ | -------- | --------------------------------- | ------------------- |
| 1   | id           | Int64    | id записи                         | 26765005            |
| 2   | created_at   | DateTime | Дата создания                     | 2026-06-23 13:13:19 |
| 3   | transaction  | Int64    | id транзакции (`transactions.id`) | 10427657            |
| 4   | account_from | Int64    | id аккаунта, откуда списано       | 6840                |
| 5   | account_to   | Int64    | id аккаунта, куда зачислено       | 10081               |
| 6   | amount       | Int64    | Сумма перевода (коп.)             | 950                 |
| 7   | currency     | Int64    | id валюты (`currencies.id`)       | 643                 |
...
| #   | Поле       | Тип          | Описание                                  | Пример              |
| --- | ---------- | ------------ | ----------------------------------------- | ------------------- |
| 1   | id         | Int64        | id транзакции                             | 3416216             |
| 2   | created_at | DateTime     | Дата создания                             | 2026-06-23 13:13:19 |
| 3   | accounts   | Array(Int64) | Список id аккаунтов-участников транзакции | [136,5476,-643]     |
...
meta_partners.legal         → legals.id
meta_partners.user          → users.id
meta_partners.partner       → partners.id

meta_partners_dim.client_id      → meta_partners.client_id
...
orders_done.done_values.transaction → transactions.id
transfers.transaction      → transactions.id
transfers.account_from     → accounts.id
transfers.account_to       → accounts.id
transfers.currency         → currencies.id
accounts.currency          → currencies.id
partners.accounts[currency]→ accounts.id   (map: код валюты → id счёта партнёра)
```
